...
public class VohnPhillip {
    ...
    /**
     * 1. reset gyros and encoders
     * 2. drive 48 inches towards the shooting position
     * 3. reset encoders
     * 4. shoot balls (try to sleep)
     * 5. rotate 151.37 degrees to face the trench run
     * 6. reset gyros
     * 7. drive 139.68 inches and stop in front of the trench run (we stop 1 foot away)
     * 8. reset encoders
     * 9. turn 28.63 degrees to face the trench
     * 10. reset gyros
     */
    public void S2HighTrench(){

        //reseting everything
        ahrs.reset();
        mDrivetrain.resetEncoders();
        //drive to shooting position
        mAutoCommands.straightDrive(48, 0.5, 500, 10000);
        //reset encoders
        mDrivetrain.resetEncoders();
        //shoot balls
        try{
        ...
        //turn to trench
        mAutoCommands.gyroTurn(151.37, 2, 500,40000);
        //reseting gyros
        ahrs.reset();
        //drive to trench (we stop 1 foot in front of trench)
        mAutoCommands.straightDrive(139.68, 0.5, 500, 40000);
        //reset encoders
        mDrivetrain.resetEncoders();
        //turn to face trench
        mAutoCommands.gyroTurn(28.63, 2, 500, 10000);
        //reseting gyros
        ahrs.reset();
 ...
 * 2. drive 1 foot forward
 * 3. reset encoders
 * 4. turn 30.14 degrees to face the shooting position
 * 5. reset gyros
 * 6. drive 95.98 inches toward the shooting position
 * 7. reset encoders
 * 8. turn 59.86 degrees to face the target
 * 9. reset gyros
 * 10. shoot (try to sleep)
 * 11. turn 151.38 degrees to face the trench
 * 12. reset gyros
 * 13. drive 139.7 inches to the trench run (we stop 1 foot away)
 * 14. reset encoders
 * 15. turn 61.38 degrees to face the trench
 * 16. reset gyros
 */
    public void S3HighTrench(){
        ...
        mAutoCommands.gyroTurn(30.14, 2, 500, 2000);
        //reseting gyros
        ahrs.reset();
        //drive to shooting position
        mAutoCommands.straightDrive(95.58, 2, 500, 4000);
        //reset encoders
        mDrivetrain.resetEncoders();
        //turn towards target
        mAutoCommands.gyroTurn(59.86, 2, 500, 2000);
        //reset gryos
        ahrs.reset();
        //shoot balls
        try{
        ...
        //turn to face trench
        mAutoCommands.gyroTurn(151.38, 2, 500, 2000);
        //reseting gyros
        ahrs.reset();
        //drive to trench (we stop 1 foot away)
        mAutoCommands.straightDrive(139.7, 2, 500, 4000);
        //reset encoders
        mDrivetrain.resetEncoders();
        //turn to face the trench
        mAutoCommands.gyroTurn(61.38, 2, 500, 2000);
        //reset gyros
        ahrs.reset();
